...
use crate::audit::{AuditEntry, OverrideRecord, OverrideType, AuditEventType};
...
impl OverrideManager {
    ...
    pub fn request_override(
        ...
        let critical_violations: Vec<_> = evaluation.all_violations()
            .iter()
            .filter(|v| v.rule_id.contains("critical") || v.rule_id.contains("emergency"))
            .collect();
        
        if !critical_violations.is_empty() && request.override_type != OverrideType::Emergency {
